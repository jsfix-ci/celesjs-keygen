...
describe('Keystore', () => {
	...
	describe('login', () => {
		it('active key (without blockchain permission)', () => {
	...
	it('low permission page login', () => {
		...
		const mypermission = PrivateKey(master.substring(2))
			.getChildKey('owner')
			.getChildKey('active')
	...
	it('uri rules history', () => {
		...
		pathname = '/'
		assert.throws(() => keystore.deriveKeys({ parent: master, accountPermissions }), /invalid login for page/)
	...
	it('saveKeyMatches disk security', () => {
		keystore = Keystore('myaccount')
		assert.throws(() => keystore.deriveKeys({ parent: master, saveKeyMatches: 'owner' }), /do not save owner key to disk/)
	})
	...
	it('signSharedSecret', () => {
		...
		PrivateKey.unsafeRandomKey().then(oneTimeServerPrivate => {
			...
			clientKeystore.signSharedSecret(oneTimeServerPublic).then(clientProof => {
				...
				const recoveredPubkeys = clientProof.signatures.map(
					signature => ecc.recover(signature, sharedSecret) // server
				)
	...
	it('wipe all', () => {
		keystore = Keystore('myaccount')
		PrivateKey.randomKey().then(privateKey => {
			keystore.addKey('active/mypermission', privateKey, true /*disk*/)
